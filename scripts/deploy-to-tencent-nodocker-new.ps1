...
# 使用方法：.\scripts\deploy-to-tencent-nodocker-new.ps1 -ServerIP "你的服务器 IP" [-Username "root"]
...
try {
    ...
    & tar --create --gzip --file=$tempFile --force-local $excludeArgs --directory=(Get-Location) .
...
$sshCommand = "set -e; cd /root; echo '正在解压文件...'; if [ -d 'tennis-link' ]; then echo '备份旧版本...'; mv tennis-link tennis-link.backup.$(date +%Y%m%d_%H%M%S); fi; mkdir -p tennis-link; tar -xzf tennis-link-deploy.tar.gz -C tennis-link; cd tennis-link; echo '安装依赖...'; npm install --production; echo '构建项目...'; npm run build; echo '配置 Nginx...'; sudo cp nginx.conf /etc/nginx/sites-available/tennis-link; sudo ln -sf /etc/nginx/sites-available/tennis-link /etc/nginx/sites-enabled/; sudo nginx -t; sudo systemctl restart nginx; echo '启动应用...'; sudo pm2 delete tennis-link 2>/dev/null || true; sudo pm2 start 'npm run start' --name 'tennis-link'; sudo pm2 save; echo '运行数据库迁移...'; npm run db:migrate || echo '数据库迁移可能需要手动执行'; rm -f /root/tennis-link-deploy.tar.gz; echo ''; echo '=========================================='; echo '部署完成！'; echo '=========================================='; echo '访问地址：http://${ServerIP}'; echo '查看日志：pm2 logs tennis-link'; echo ''"
